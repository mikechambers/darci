...
[Full Player Details](images/player_details.png) | [Full Activity Details](images/activity_details.png) | [Full Compare View](images/compare_details.png) | [Stream Overlay](images/stream_overlay.png) | [Trials Stream Overlay](images/trials_overlay.png)
...
Go to Bungie and register for a [Destiny 2 API key](https://www.bungie.net/en/Application). Use the following settings:
...
Create two environment variables named `DESTINY_API_KEY` and
...
and to enable the web frontend of DARCI to call the Destiny 2 APIs directly.
...
Download the latest release of [dcli](https://github.com/mikechambers/dcli) and place the
files in your system path so they can be called via the command line.
...
will make additional API calls when syncing data if there is missing data from the API
(which happens sometimes). This can significantly slow down the initial sync, but is
recommended as it can help prevent missing data.

#### Sync Player Data

At this point, we are ready to do the initial data sync. The first sync can take
some time, and thus it's suggested you sync one player first to make sure
...
variable correctly, run the command again. (Sometimes some API calls will time out.
This is normal and no data will be lost. It will be correctly synced on subsequent calls).
...
Next, we need to schedule the player data syncing via `dclisync`. There are two ways to do this:
...
We have had a good experience with using NGINX web server as a reverse proxy in front of Node (which is run and managed by the PM2 process manager). In this setup the React-based front end is actually served through the Node server, and proxied via NGINX. You also should set up an SSL cert for your server if you will be running it publicly.
...
Note, once you have the server setup, you will see an error until you create a production build of the app (see next).

#### Create a Production Build of the App

Once your have the server setup and configured you need to create a production build of the React front end. You can do this by changing into the `client-web` directory and running:
...
1. The data you want to analyze, either activity history, or a specific activity
2. A copy of the manifest generated by DARCI to match IDs with names.

For activity history, call the DARCI server with the following URL:

`/api/player/activities/MEMBER_ID/CLASS_TYPE/MODE/START_PERIOD/END_PERIOD/?limit=TOP_NUMBER_OF_RESULTS_TO_RETURN`

For example, this URL:

`/api/player/activities/4611686018429783292/All/TrialsOfOsiris/SeasonOfTheSeraph/Now/?limit=10000`

Returns JSON data for the last 10000 activities for all classes in Trials of Osiris since Season of the Seraph began.

You can grab a copy of the generated manifest that DARCI uses from:

`/manifest/check/`

You can then upload the data to ChatGPT with the following instructions:

`The file (activities.json) contains Destiny 2 PVP data for a single player for the trials of osiris game mode. A standing of 0 is a win, and a standing of 1 is a loss. IDs in the data can be looked up in the manifest.json file which I will upload next.`

Once you have done this, you can just ask ChatGPT to analyze the data and give you insights.
